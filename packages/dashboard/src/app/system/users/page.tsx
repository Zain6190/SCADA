...
function ScopeLabel({ user }: { user: AdminUser }) {
  ...
  if (!scope?.access) return <span className="text-ink-subtle">No scope</span>
...
function SummaryBox({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-line bg-canvas p-4">
      ...
      <p className="mt-1 text-[11px] uppercase tracking-wider text-ink-subtle">{label}</p>
    </div>
  )
}
  ...
  return (
    <tr className="transition-colors hover:bg-surface-alt">
      <td className="px-5 py-3">
        ...
        <p className="font-mono text-[11px] text-ink-subtle">{user.email}</p>
      </td>
      <td className="px-5 py-3">
      ...
      <td className="px-5 py-3">
        <p className="font-mono text-[10px] leading-4 text-ink-subtle">
      ...
      <td className="px-5 py-3 text-right">
        <Link
          ...
          className="inline-flex items-center gap-1 rounded-lg border border-line-strong bg-surface-alt px-3 py-1.5 text-xs font-medium text-ink transition-colors hover:bg-surface-sunken"
        >
