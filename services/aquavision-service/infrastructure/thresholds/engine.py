...
def _open_alert_exists(db: Session, asset_id: int, alert_type: str) -> bool:
    ...
    result = db.execute(
        select(func.count(WaterOperationalAlert.id)).where(
            ...
            WaterOperationalAlert.status.notin_(
                [STATUS_RESOLVED, STATUS_FALSE_INVALID, "AUTO_RESOLVED"]
            ),
        )
    ).scalar()
    return result > 0
...
def _create_alert(
    ...
    # ACK SLA: CRITICAL 15 min, WARNING 4 h (see infrastructure/alerts/workflow.py)
    from infrastructure.alerts.workflow import sla_due_at as _sla_due_at
    alert.sla_due_at = _sla_due_at(severity, datetime.now(timezone.utc))
    db.add(alert)
    db.flush()
    ...
    audit = WaterAlertAuditLog(
        ...
        payload={"alert_source": alert_source, "sla_due_at": alert.sla_due_at.isoformat() if alert.sla_due_at else None},
    )
    db.add(audit)
    db.flush()
...
def _dispatch_notifications(db: Session, alerts: List[WaterOperationalAlert]) -> None:
    ...
    # Parse recipients from env + role-based recipients (supervisors/admins)
    recipients = [r.strip() for r in settings.ALERT_RECIPIENTS.split(",") if r.strip()]
    try:
        from infrastructure.alerts.workflow import role_emails
        recipients = sorted(set(recipients + role_emails(db, ["water_supervisor", "admin"])))
    except Exception as e:
        logger.warning(f"Role-based recipients unavailable: {e}")
    if not recipients:
