...
from infrastructure.db.models import Asset, Permission, Region, Role, RolePermission, User, UserRole, UserRegionScope
from infrastructure.audit import log_security_event
...
# ---------------------------------------------------------------------------
def get_active_scopes(user_id: int, db) -> List[UserRegionScope]:
    """All currently-granted scopes for a user (active, not expired)."""
    now = datetime.now(UTC)
    rows = db.execute(
        select(UserRegionScope).where(
            UserRegionScope.user_id == user_id,
            UserRegionScope.is_active.is_(True),
            or_(
                UserRegionScope.expires_at.is_(None),
                UserRegionScope.expires_at > now,
            ),
        )
    ).scalars().all()
    return list(rows)
...
def _region_descendants(region_id: int, db) -> List[int]:
    ...
    while frontier:
        children = db.execute(
            select(Region.id).where(Region.parent_region_id.in_(frontier))
        ).scalars().all()
...
def _resolve_scope(scopes: List[UserRegionScope], db) -> Optional[List[int]]:
...
def require_permissions(*required: str):
    ...
    """
    async def checker(token: str = Depends(oauth2_scheme), db=Depends(get_session)):
...
def get_db_user(token: str = Depends(oauth2_scheme), db=Depends(get_session)) -> User:
...
def require_admin(user: User = Depends(get_db_user), db=Depends(get_session)) -> User:
