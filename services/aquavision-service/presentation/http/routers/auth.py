...
# Authentication + user administration against PostgreSQL (shared.users +
# RBAC). Ported from packages/backend auth endpoints; tokens are issued via
# infrastructure.auth.jwt (PyJWT), passwords verified with bcrypt.
import re
...
from infrastructure.audit import log_security_event, write_audit
...
from infrastructure.auth.schemas import (
    OperatorUpdateRequest,
    UserUpdateRequest,
    _ACCESS_STATUSES,
    _PASSWORD_MIN,
)
from infrastructure.auth.security import get_password_hash, verify_password
...
from presentation.http.rate_limit import limiter
...
def _check_access_allowed(user: User) -> None:
...
def _user_payload(user: User, db) -> dict:
...
async def login(request: Request, body: LoginRequest, db=Depends(get_session)):
    ...
    admin@ibcp.gov.pk). Rate limited: 5 attempts per minute per IP.
    """
    ident = body.username.strip().lower()
    if "@" in ident:
        stmt = select(User).where(func.lower(User.email) == ident)
    else:
        escaped = (
            ident.replace("\\", "\\\\").replace("%", "\\%").replace("_", "\\_")
        )
        stmt = select(User).where(
            func.lower(User.email).like(escaped + "@%", escape="\\")
        )
    user = db.execute(stmt.order_by(User.id).limit(1)).scalars().first()
    if user is None or not verify_password(body.password, user.password_hash):
        log_security_event(
            action="AUTH_FAILED_LOGIN",
            user_id=user.id if user else None,
            details={"email": body.username},
        )
        raise HTTPException(
    ...
    token = create_token(
        ...
        role=payload["role"],
        roles=roles,
        extra={"email": user.email, "permissions": payload["permissions"]},
    )
...
def _replace_user_scope(db, admin: User, target: User, scope: dict) -> None:
    """Replace a user's geographic scope rows with the submitted value.

    scope: {scope_type, region_id|asset_id, expires_at}
    ...
    """
    old = list(db.execute(
        select(UserRegionScope).where(UserRegionScope.user_id == target.id)
    ).scalars().all())
    for s in old:
        db.delete(s)
    db.flush()
...
def _user_before(user: User, db) -> dict:
    roles = db.execute(
        select(Role.name)
        .join(UserRole, UserRole.role_id == Role.id)
        .where(UserRole.user_id == user.id)
...
def _user_after(user: User, db) -> dict:
...
async def update_user(
    ...
    if role_name is not None:
        if role_name.lower() in _FORBIDDEN_ROLES:
            raise HTTPException(status_code=400, detail=f"Role '{role_name}' cannot be assigned")
        for ur in db.execute(select(UserRole).where(UserRole.user_id == target.id)).scalars():
            db.delete(ur)
        db.flush()
        db.add(UserRole(user_id=target.id, role_id=_role_id(db, role_name)))
...
async def list_operator_roles(actor: dict = Depends(MANAGE_OPERATORS), db=Depends(get_session)):
    ...
    rows = db.execute(
        select(Role).where(Role.name.in_(names)).order_by(Role.name)
    ).scalars().all()
    return [
        {
            "name": r.name,
...
async def list_operators(actor: dict = Depends(MANAGE_OPERATORS), db=Depends(get_session)):
    ...
    users = db.execute(
        ...
        .where(Role.name.in_(sorted(_DELEGATED_ROLES)))
