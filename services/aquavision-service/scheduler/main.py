# services/aquavision-service/scheduler/main.py
...
# Lives inside services/aquavision-service/scheduler/ (single-backend layout).
import sys
import os
import time
import logging
...
def _find_ml_root() -> str:
    """Locate services/ml-pipeline (repo layout) or /ml-pipeline (container)."""
    candidates = [
        os.environ.get("ML_PIPELINE_ROOT", ""),
        os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "ml-pipeline")),
        "/ml-pipeline",
    ]
    for c in candidates:
